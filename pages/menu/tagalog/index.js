...
import TagalogMenu from "./create";
import { FaChevronLeft, FaChevronRight } from "react-icons/fa";
import Footer from "../../../components/Footer/Footer";
import { FaStar } from "react-icons/fa";

const Tagalog = () => {
  const [tagalogItems, setTagalogItems] = useState([]);
  ...
  useEffect(() => {
    ...
    setPaginatedItems(tagalogItems.slice(indexOfFirstItem, indexOfLastItem));
  }, [currentPage, itemsPerPage, tagalogItems]);
      ...
      >
        ...
        <div className="container mt-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
            {paginatedItems.map((tagalog) => (
              <Link key={tagalog.id} href={`/menu/tagalog/${tagalog.id}`}>
                  ...
                  <div className="bg-[#212121] text-white text-center shadow-md rounded-lg p-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex justify-start items-center">
                        {renderStars(tagalog.rating)}
                      </div>
                    </div>
                    <p className="text-start font-sembold mt-2">
                      ₱
                      {typeof tagalog.price === "number"
                        ? tagalog.price.toFixed(2)
                        : parseFloat(tagalog.price).toFixed(2)}
                    </p>
                  </div>
                </div>
              </Link>
          ...
          <div className="flex flex-col items-center md:items-end md:w-full mt-4 mb-2 mr-4 md:mb-0">
            <div className="bg-[#212121] flex justify-center md:justify-end mt-4 mb-2 md:mt-0 p-3 rounded-lg">
              ...
              <button
                onClick={nextPage}
                disabled={
                  currentPage === Math.ceil(tagalogItems.length / itemsPerPage)
                }
                className={`px-4 py-2 ${
                  currentPage === Math.ceil(tagalogItems.length / itemsPerPage)
              ...
              <span className="ml-4 text-white">
                ...
                - {Math.min(currentPage * itemsPerPage, tagalogItems.length)} of{" "}
                {tagalogItems.length} items
              </span>
            </div>
          </div>
